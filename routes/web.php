...
// Primer login y bienvenida (solo auth)
Route::middleware(['auth'])->group(function () {
    Route::get('/bienvenido', function () { return view('auth.welcome'); })->name('welcome');
...
// App (auth + profile.completed + force.password.change)
Route::middleware(['auth', 'profile.completed', 'force.password.change'])->group(function () {
    ...
    Route::get('/videos/{id}/stream', function ($id) {
        ...
        if (!$video) return response('Not Found', 404)->header('Content-Type', 'text/plain');
        if ($video->album_type === 'private') {
            if (!\App\Services\MembershipService::can($user->id, 'can_view_private_photos'))
                return response('Forbidden: necesitas membresia Connectors.', 403)->header('Content-Type', 'text/plain');
        }
        if ($video->album_type === 'vip') {
            if (!\App\Services\MembershipService::hasMinLevel($user->id, 'vip_elite'))
                return response('Forbidden: necesitas membresia VIP Elite.', 403)->header('Content-Type', 'text/plain');
        }
        if ($video->status !== 'approved' && (string)$video->user_id !== (string)$user->id)
            return response('Forbidden: video pendiente.', 403)->header('Content-Type', 'text/plain');
        $cacheKey = 'stream_url_' . $video->id . '_' . $user->id;
        try {
            $signedUrl = \Illuminate\Support\Facades\Cache::remember($cacheKey, now()->addMinutes(4), function () use ($video) {
                return \Illuminate\Support\Facades\Storage::disk('supabase')->temporaryUrl($video->file_path, now()->addMinutes(5));
            });
        } catch (\Throwable $e) {
            $localPath = storage_path('app/private/' . ltrim($video->file_path, '/'));
            if (!file_exists($localPath)) return response('Not Found', 404)->header('Content-Type', 'text/plain');
            $signedUrl = null;
        }
        if ($signedUrl) return redirect($signedUrl);
        $path = storage_path('app/private/' . ltrim($video->file_path, '/'));
        $ext  = strtolower(pathinfo($path, PATHINFO_EXTENSION));
        $mime = ['mp4'=>'video/mp4','mov'=>'video/quicktime','avi'=>'video/x-msvideo','webm'=>'video/webm'][$ext] ?? 'video/mp4';
        $size = filesize($path);
        $start = 0; $end = $size - 1; $status = 200;
        $headers = ['Content-Type'=>$mime,'Accept-Ranges'=>'bytes','Cache-Control'=>'private, max-age=3600'];
        ...
        return response()->stream(function () use ($path, $start, $length) {
            ...
            $f = fopen($path, 'rb'); fseek($f, $start); $rem = $length;
            while (!feof($f) && $rem > 0) { $chunk = min(65536, $rem); echo fread($f, $chunk); $rem -= $chunk; flush(); }
            fclose($f);
        }, $status, $headers);
    })->name('videos.stream');
