        ...
        <li class="l69-sidebar-nav__item">
            <a href="{{ route('messages.index', ['tab' => $key]) }}"
               class="{{ $tab === $key ? 'is-active' : '' }}">
                ...
                @elseif($key === 'comments' && ($unreadComments ?? 0) > 0)
                    <span style="margin-left:auto;background:#e74c3c;color:#fff;font-size:.65rem;font-weight:700;border-radius:99px;padding:1px 6px;">{{ $unreadTotal }}</span>
                @endif
            </a>
        </li>
...
<div class="l69-msg-wrap">
  ...
  <div class="l69-msg-body">
    ...
    {{-- ─── COMENTARIOS ─── --}}
    @if($tab === 'comments')
    <div class="l69-inbox-header">
        <h2 class="l69-msg-title" style="margin:0;">🗨️ Comentarios en tus fotos</h2>
        @if(($unreadComments ?? 0) > 0)
            <span class="l69-inbox-unread-badge">{{ $unreadComments }} nuevos</span>
        @endif
    </div>

    @forelse($photoComments as $pc)
    @php $isUnread = is_null($pc->read_at); @endphp
    <div class="l69-inbox-row {{ $isUnread ? 'is-unread' : '' }}">
        ...
        <div class="l69-inbox-avatar">
            ...
            @endif
            @if($isUnread)<span class="l69-inbox-dot"></span>@endif
        </div>

        {{-- Contenido --}}
        <div class="l69-inbox-body">
            <div class="l69-inbox-top">
                <a href="/u/{{ $pc->commenter_nick }}"
                   class="l69-inbox-name">{{ $pc->commenter_nick ?? $pc->commenter_name }}</a>
                <span class="l69-inbox-time">{{ \Carbon\Carbon::parse($pc->created_at)->diffForHumans() }}</span>
            </div>
            <div class="l69-inbox-text {{ $isUnread ? 'fw-bold' : '' }}">
                {{ $pc->body }}
            </div>
            <div class="l69-inbox-sub">
                En:&nbsp;<a href="#"
                   class="l69-comment-photo-link l69-inbox-photo-link"
                   data-photo-id="{{ $pc->photo_uuid }}">
                    <i class="fas fa-image" style="font-size:.6rem;"></i>
                    {{ Str::limit($pc->caption ?? 'Sin título', 30) }}
                </a>
            </div>
        </div>

        {{-- Acción rápida --}}
        <div class="l69-inbox-actions">
            <button class="l69-inbox-btn-view l69-comment-photo-link"
                    data-photo-id="{{ $pc->photo_uuid }}"
                    title="Ver foto">
    ...
    @empty
        <div class="l69-inbox-empty">
            <i class="fas fa-comment-slash"></i>
            <p>Aún no tienes comentarios en tus fotos.</p>
        </div>
    @endforelse

    @if($videoComments->isNotEmpty())
    <div class="l69-inbox-header" style="margin-top:1.5rem;">
        <h2 class="l69-msg-title" style="margin:0;">🎬 Comentarios en videos</h2>
    </div>
    @foreach($videoComments as $vc)
    <div class="l69-inbox-row">
        <div class="l69-inbox-avatar">
            <div class="l69-avatar-ph">{{ mb_substr($vc->commenter_name ?? '?', 0, 1) }}</div>
        </div>
        <div class="l69-inbox-body">
            <div class="l69-inbox-top">
                <a href="/u/{{ $vc->commenter_nick }}" class="l69-inbox-name">
                    {{ $vc->commenter_nick ?? $vc->commenter_name }}
                </a>
                <span class="l69-inbox-time">{{ \Carbon\Carbon::parse($vc->created_at)->diffForHumans() }}</span>
            </div>
            <div class="l69-inbox-text">{{ $vc->body }}</div>
            <div class="l69-inbox-sub">En: <em>{{ Str::limit($vc->title ?? 'Video', 30) }}</em></div>
        </div>
    </div>
    @endforeach
    @endif
...
/* ── Inbox rows (Sub-tema C) ── */
.l69-inbox-header               { display:flex;align-items:center;gap:.75rem;margin-bottom:.5rem;padding-bottom:.4rem;border-bottom:2px solid rgba(180,60,120,.15); }
.l69-inbox-unread-badge         { background:var(--theme-pink,#e056a0);color:#fff;font-size:.65rem;font-weight:700;border-radius:99px;padding:2px 8px; }

.l69-inbox-row                  { display:flex;align-items:center;gap:.75rem;padding:.55rem .75rem;border-radius:10px;background:var(--theme-card,rgba(255,255,255,.04));margin-bottom:.35rem;transition:background .15s;position:relative; }
.l69-inbox-row:hover            { background:rgba(224,86,160,.07); }
.l69-inbox-row.is-unread        { background:rgba(224,86,160,.10);border-left:3px solid var(--theme-pink,#e056a0); }

.l69-inbox-avatar               { position:relative;flex-shrink:0; }
.l69-inbox-avatar img,
.l69-inbox-avatar .l69-avatar-ph{ width:40px;height:40px;border-radius:50%;object-fit:cover; }
.l69-inbox-dot                  { position:absolute;top:0;right:0;width:10px;height:10px;background:#e74c3c;border-radius:50%;border:2px solid var(--theme-bg,#1a1a2e); }

.l69-inbox-body                 { flex:1;min-width:0; }
.l69-inbox-top                  { display:flex;align-items:baseline;gap:.5rem;margin-bottom:.1rem; }
.l69-inbox-name                 { font-weight:700;font-size:.82rem;color:var(--theme-text);text-decoration:none;white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }
.l69-inbox-name:hover           { color:var(--theme-pink,#e056a0); }
.l69-inbox-time                 { margin-left:auto;font-size:.68rem;color:var(--theme-muted);white-space:nowrap;flex-shrink:0; }
.l69-inbox-text                 { font-size:.8rem;color:var(--theme-text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:.15rem; }
.l69-inbox-text.fw-bold         { font-weight:700;color:var(--theme-text); }
.l69-inbox-sub                  { font-size:.7rem;color:var(--theme-muted); }
.l69-inbox-photo-link           { color:var(--theme-pink,#e056a0);text-decoration:none; }
.l69-inbox-photo-link:hover     { text-decoration:underline; }

.l69-inbox-actions              { flex-shrink:0;display:flex;flex-direction:column;gap:.3rem; }
.l69-inbox-btn-view             { background:rgba(224,86,160,.15);border:none;color:var(--theme-pink,#e056a0);border-radius:6px;padding:.3rem .5rem;cursor:pointer;font-size:.8rem;transition:background .15s; }
.l69-inbox-btn-view:hover       { background:rgba(224,86,160,.3); }

.l69-inbox-empty                { text-align:center;padding:2.5rem 1rem;color:var(--theme-muted); }
.l69-inbox-empty i              { font-size:2rem;margin-bottom:.5rem;display:block;opacity:.4; }
.l69-inbox-empty p              { margin:0;font-size:.85rem; }
