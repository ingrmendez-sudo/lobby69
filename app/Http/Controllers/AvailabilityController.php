    ...
    public function publicList(\Illuminate\Http\Request $request)
    {
        ...
        $query = \Illuminate\Support\Facades\DB::table('availability as av')
            ->join('users as u',
                ...
                \Illuminate\Support\Facades\DB::raw('av.user_id::text'))
            ->leftJoin('profiles as p',
                \Illuminate\Support\Facades\DB::raw('p.user_id::text'), '=',
                \Illuminate\Support\Facades\DB::raw('u.id::text'))
            ->leftJoin(
        ...
        if ($slot) {
            $query->where('av.slot', $slot);
        }

        if ($search) {
            $query->where(function ($q) use ($search) {
                $q->where('p.nickname', 'ilike', '%' . $search . '%')
